...
class DynamicScaleNegAugCLIPTextGenerator:
    ...
    def choose_neg_features(self, image_feature, clip_embeddings, verbose=False):
        sim = image_feature @ clip_embeddings.T
        neg_idxs = sim.argsort()[0][:self.num_neg]
        if verbose:
            avg_neg_sim = sim.T[neg_idxs].mean()
            print(f"Avg Neg Sim: {avg_neg_sim.item()}")
        return clip_embeddings[neg_idxs]

    def generate(self, image_features: torch.Tensor, clip_embeddings, verbose=True):
        self.image_features = image_features
        neg_features = self.choose_neg_features(image_features, clip_embeddings, verbose=verbose)
        ...
        for gd_iter in range(self.sentence_iterations):
            ...
            for i in range(self.target_seq_length):
                ...
                # compute neg loss
                neg_clip_loss, _ = self.neg_clip_loss(neg_features, probs, generated_tokens, 512)

                # scheduled scale
                progress = float(i) / float(max(1, self.target_seq_length))
                clip_scale_ = (0.5 + max(0.0, 0.5 * (1. + math.cos(math.pi * float(0.5) * 2.0 * progress))))
                # Take gradients
                loss = (clip_scale_ * self.clip_scale * clip_loss) + (self.ce_scale * fluency_loss) - (clip_scale_ * self.neg_scale * neg_clip_loss)
                loss.backward()
    ...
    def neg_clip_loss(self, neg_features, probs, context_tokens, samples: int):
        _, top_next_tokens = probs.topk(samples, -1)
        ...
        for prefix_text, prefix_top_next_tokens, prefix_next_token_prob in zip(prefix_texts, top_next_tokens, probs):
            ...
            with torch.no_grad():
                text_features = self.get_txt_features(top_prefix_continuation)
                cont_video_similarity = self.logit_scale * (text_features @ neg_features.T).type(torch.float32)
